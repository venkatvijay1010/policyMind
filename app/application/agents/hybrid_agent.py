...
class HybridAgent:
    ...
    async def _extract_sub_queries(self, query: str) -> dict:
        """
  ...
  "needs_sql": true/false
        ...
        try:
            result = await self.llm.generate_json(
                prompt=prompt,
                system_prompt="You are an expert at analyzing insurance queries."
            )
            return result
...
"""
    ...
    async def answer(
        ...
        if sub_queries.get("needs_doc", True):
        ...
        if sub_queries.get("needs_sql", True):
            sql_query_text = sub_queries.get("sql_query", query)
