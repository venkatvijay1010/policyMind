...
def _percentile(sorted_values: List[int], percentile: float) -> float:
    """Return a linearly interpolated percentile without a heavy numerical dependency."""
...
def compute_retrieval_metrics(
    retrieved_ids: List[int], relevant_ids: List[int], k: int = 10
) -> EvaluationMetrics:
    """
...
def compute_answer_quality(
    expected_answer: str, actual_answer: str, key_terms: Optional[List[str]] = None
) -> dict:
    """
    ...
    return {
        ...
        "overall_score": round(
            (similarity * 0.5 + key_term_coverage * 0.3 + length_score * 0.2), 4
        ),
    }
...
def compute_latency_metrics(latencies_ms: List[int]) -> dict:
    ...
    return {
        ...
        "median": round(median(sorted_latencies), 2),
        "p95": round(_percentile(sorted_latencies, 95), 2),
        "p99": round(_percentile(sorted_latencies, 99), 2),
    }
