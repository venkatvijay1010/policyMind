...
class Policy(Base):
    ...
    # Relationships
    chunks: Mapped[List["ContractPassage"]] = relationship(
        "ContractPassage", back_populates="policy", cascade="all, delete-orphan"
    )
    plan_benefits: Mapped[List["Coverage"]] = relationship(
        "Coverage", back_populates="policy", cascade="all, delete-orphan"
    )
    participants: Mapped[List["Member"]] = relationship(
        "Member", back_populates="policy", cascade="all, delete-orphan"
    )
    service_cases: Mapped[List["Claim"]] = relationship(
...
class ContractPassage(Base):
    ...
    contract_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("benefit_contracts.id", ondelete="CASCADE")
    )
    ...
    # SQLite stores vectors as JSON. HybridSearch performs cosine ranking in
    # Python, which is appropriate for the small local/demo knowledge base.
...
class EvalQuestion(Base):
    ...
    relevant_contract_ids: Mapped[Optional[List[int]]] = mapped_column(JSON)
    relevant_chunk_ids: Mapped[Optional[List[int]]] = mapped_column(JSON)
...
class EvalResult(Base):
    ...
    retrieved_chunk_ids: Mapped[Optional[List[int]]] = mapped_column(JSON)
