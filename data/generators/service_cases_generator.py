...
def generate_participants(
    ...
    states = ["North District", "West District", "Central District", "Coastal District", "East District", "South District"]
    enrolment_roles = ["SELF", "SPOUSE", "CHILD", "PARENT"]
    
    for i in range(count):
        enrolment_role = random.choices(
            enrolment_roles,
            weights=[40, 30, 20, 10],
            k=1
        )[0]
        
        age = random.randint(25, 60) if enrolment_role == "SELF" else (
            random.randint(25, 55) if enrolment_role == "SPOUSE" else
            random.randint(1, 22) if enrolment_role == "CHILD" else
